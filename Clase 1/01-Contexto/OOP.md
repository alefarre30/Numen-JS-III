...
_.extend(santi, guille, cristian);
```

Lo que hace `extend`, es combinar todas los métodos y propiedades de `guille` y `cristian` dentro del objeto `santi`.  Si ahora vemos el objeto `santi`, vemos que conserva sus propiedades y métodos, pero ahora tiene las de `guille` y `cristian`. Y, a diferencia de haberlas obtenido a traves del `prototype chain`, _estas propiedades y métodos_ __son__ de `santi`.
...
var cristian = new Person('Cristian', 'Tralice');
...
var cristian = new Persona('Cristian', 'Tralice');
cristian.saludar();
```
...
var cristian = new Empleado('Cristian', 'Tralice', 'Profesor', 100);
cristian.saludar();
```
